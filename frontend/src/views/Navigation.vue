  ...
  <div class="nav-page">
    ...
    <header class="top-bar">
      <div class="top-content">
        <div class="top-left">
          <RivenLogo clickable @click="scrollToSection(categories[0]?.key)" />
        </div>
        <div class="top-right">
    ...
    <div class="body-area">
      ...
      <aside class="sidebar">
        <nav class="sidebar-nav">
          ...
          >
            ...
            <span class="sidebar-count" v-if="sectionData[cat.key]?.length">{{ sectionData[cat.key].length }}</span>
          </button>
        </nav>
      </aside>
      ...
      <main class="scroll-content" ref="scrollContainer" @scroll="handleScroll">
        <div class="content-wrapper">
          <!-- 内部项目色块区域 -->
          <section class="featured-section" v-if="featuredProjects.length">
            <div class="featured-grid">
              ...
              >
                <div class="featured-card__icon">
                  <img v-if="project.icon_image" :src="project.icon_image" :alt="project.name" />
                  <span v-else-if="project.icon_emoji">{{ project.icon_emoji }}</span>
                  <el-icon v-else size="32" color="rgba(255,255,255,0.9)"><Monitor /></el-icon>
                </div>
                <div class="featured-card__body">
                  <h3 class="featured-card__title">{{ project.name }}</h3>
                  <p class="featured-card__desc">{{ project.description || '进入项目' }}</p>
                </div>
                <el-icon class="featured-card__arrow" size="20"><ArrowRight /></el-icon>
              </div>
            </div>
          </section>
          ...
          >
            <div class="section-header">
              <h2 class="section-title">
                ...
                <span class="section-title__text">{{ cat.label }}</span>
                <span class="section-title__line"></span>
              </h2>
              <el-button
            ...
            <div class="links-grid" v-loading="sectionLoading[cat.key]">
              ...
              >
                ...
                <div class="card-admin" v-if="userStore.isAdmin && cat.key !== 'favorites'">
                  ...
                  <button class="admin-btn fetch-btn" title="抓取图标" @click.stop="handleFetchIcon(link)">
                    <el-icon size="14"><Download /></el-icon>
                  </button>
                  <button class="admin-btn delete-btn" title="删除" @click.stop="handleDelete(link, cat.key)">
    ...
    >
      <el-form :model="linkForm" label-width="80px" label-position="left">
        <el-form-item label="名称" required>
          <el-input v-model="linkForm.name" placeholder="网站名称" maxlength="100" />
        </el-form-item>
        <el-form-item label="链接" required>
          <el-input v-model="linkForm.url" placeholder="https://example.com 或 /internal-path" />
        </el-form-item>
        <el-form-item label="描述">
          <el-input v-model="linkForm.description" type="textarea" :rows="2" placeholder="简短描述（可选）" />
        </el-form-item>
        ...
        <el-form-item label="类型">
          <el-switch v-model="linkForm.is_internal" active-text="内部链接" inactive-text="外部链接" />
        </el-form-item>
          ...
          <div class="icon-upload-row">
            ...
            >
              <div class="icon-uploader">
                ...
                <el-icon v-else class="icon-upload-placeholder" size="24"><Plus /></el-icon>
              </div>
            </el-upload>
            <el-input v-model="linkForm.icon_emoji" placeholder="或输入 Emoji" style="width: 140px; margin-left: 12px" />
            <span class="form-hint">上传图片或输入 Emoji</span>
          </div>
        </el-form-item>
        <el-form-item label="排序">
          <el-input-number v-model="linkForm.sort_order" :min="0" :max="9999" />
...
.featured-card {
  ...
  &--iqair {
    background: var(--gradient-iqair);
  }
  &--ciyun {
    background: var(--gradient-ciyun);
  }
  &--bi {
